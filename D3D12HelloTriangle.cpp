...
{
  // Before Win10 RS5, raytracing and vs/ps_6_0 are in prototype state, which has to be activated
  // explicitly
  ...
  ThrowIfFailed(m_device->QueryInterface(IID_PPV_ARGS(&m_rtDevice)));
  ...
  // Command lists are created in the recording state, but there is nothing to record yet. The main
  // loop expects it to be closed, so close it now.
  ThrowIfFailed(m_commandList->Close());

  // Create the raytracing pipeline, associating the shader code to symbol names and to their root
  // signatures, and defining the amount of memory carried by rays (ray payload)
  ...
  // Create the buffer containing the raytracing result (always output in a UAV), and create the
  // heap referencing the resources used by the raytracing, such as the acceleration structure
  CreateShaderResourceHeap(); // #DXR

  // Create the shader binding table and indicating which shaders are invoked for each instance in
  // the  AS
...
#pragma warning(pop)
  ...
  // Create synchronization objects and wait until assets have been uploaded to the GPU.
  {
    ...
    // Wait for the command list to execute; we are reusing the same command list in our main loop
...
{
  // Ensure that the GPU is no longer referencing resources that are about to be cleaned up by the
  // destructor.
  WaitForPreviousFrame();
...
void D3D12HelloTriangle::PopulateCommandList()
{
  ...
  // However, when ExecuteCommandList() is called on a particular command list, that command list
  // can then be reset at any time and must be before re-recording.
...
#pragma warning(pop)
  ...
  else
  {
    ...
    // On the last frame, the raytracing output was used as a copy source, to copy its contents into
    // the render target. Now we need to transition it to a UAV so that the shaders can write in it.
    ...
    // target buffer to a copy destination. We can then do the actual copy, before transitioning the
    // render target buffer into a render target, that will be then used to display the image
...
void D3D12HelloTriangle::WaitForPreviousFrame()
{
  ...
  // This is code implemented as such for simplicity. The D3D12HelloFrameBuffering sample
  // illustrates how to use fences for efficient resource usage and to maximize GPU utilization.
    ...
    std::vector<std::pair<ComPtr<ID3D12Resource>, uint32_t>> vVertexBuffers)
{
  ...
  {
    bottomLevelAS.AddVertexBuffer(buffer.first.Get(), 0, buffer.second, sizeof(Vertex), nullptr, 0);
  }
  ...
  bottomLevelAS.ComputeASBufferSizes(m_rtDevice, false, &scratchSizeInBytes, &resultSizeInBytes);
  ...
  buffers.pScratch = nv_helpers_dx12::CreateBuffer(
      m_device.Get(), scratchSizeInBytes, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS,
      D3D12_RESOURCE_STATE_COMMON, nv_helpers_dx12::kDefaultHeapProps);
  buffers.pResult = nv_helpers_dx12::CreateBuffer(
      m_device.Get(), resultSizeInBytes, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS,
...
)
{
  ...
  m_topLevelASGenerator.ComputeASBufferSizes(m_rtDevice, true, &scratchSize, &resultSize,
                                           &instanceDescsSize);
...
//--------------------------------------------------------------------------------------------------
//
// Combine the BLAS and TLAS builds to construct the entire acceleration structure required to
// raytrace the scene
//
...
//--------------------------------------------------------------------------------------------------
// The ray generation shader needs to access 3 resources: the raytracing output, the top-level
// acceleration structure and the constant buffer containing the camera information
//
...
//--------------------------------------------------------------------------------------------------
// The hit shaders communicates only through the ray payload, and therefore does not require any
// resources
//
...
//--------------------------------------------------------------------------------------------------
// The miss shaders communicates only through the ray payload, and therefore does not require any
// resources
//
...
{
  nv_helpers_dx12::RayTracingPipelineGenerator pipeline(m_device.Get(), m_rtDevice);
...
//--------------------------------------------------------------------------------------------------
//
// Create the main heap used by the shaders, which will give access to the raytracing output, the
// top-level acceleration structure, and the camera matrices
//
...
void D3D12HelloTriangle::CreateShaderBindingTable()
{
  ...
  uint32_t sbtSize = m_sbtHelper.ComputeSBTSize(m_rtDevice);
