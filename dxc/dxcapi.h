...
IDxcVsPsToHitShader : public IUnknown {
  // Convert vertex and pixel shader pair to closest-hit and/or any-hit raytracing shaders.
  virtual HRESULT STDMETHODCALLTYPE Transform(
    _In_ IDxcBlob              *pVsShader,                      // Vertex shader input
    _In_ IDxcBlob              *pPsShader,                      // Pixel shader input
    DxcInputLayoutDesc          inputLayout,                    // Specifies the mapping from vertex buffers to vertex shader inputs
    UINT                        indexByteSize,                  // Size in bytes of a single index in the index buffer (4, 2, or 0)
    const UINT                 *pVertexBufferStrides,           // Stride in bytes of each vertex buffer
    UINT                        iaRegisterSpace,                // Register space to use for index and vertex buffers
    UINT                        vsRegisterSpaceAddend,          // Register space value to add to vertex shader registers
    UINT                        psRegisterSpaceAddend,          // Register space value to add to pixel shader registers
    UINT64                      chRendertargetScalarOutputMask, // Specifies pixel shader outputs that should be stored in payload for closest-hit
    UINT64                      ahRendertargetScalarOutputMask, // Specifies pixel shader outputs that should be stored in payload for any-hit
